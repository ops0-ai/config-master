...
setup_environment() {
    ...
    # Allow override via environment variable for self-hosted deployments
    if [ -n "$PULSE_SERVER_IP" ]; then
        SERVER_IP="$PULSE_SERVER_IP"
        log "🌐 Using provided server IP: $SERVER_IP"
    else
        SERVER_IP="localhost"
        log "🏠 Using localhost for local development"
        
        # Only try to detect external IP if explicitly requested
        if [ "$PULSE_AUTO_DETECT_IP" = "true" ]; then
            EXTERNAL_IP=$(curl -s ifconfig.me 2>/dev/null || curl -s ipinfo.io/ip 2>/dev/null || echo "")
            if [ -n "$EXTERNAL_IP" ]; then
                SERVER_IP="$EXTERNAL_IP"
                log "🌐 Auto-detected external IP: $SERVER_IP"
            fi
        fi
    fi
...
# API URL - Uses detected server IP for browser access
NEXT_PUBLIC_API_URL=http://$SERVER_IP:5005/api
...
EOF
    ...
    # Update docker-compose.yml to use the correct API URL
    log "Updating docker-compose environment..."
    export NEXT_PUBLIC_API_URL="http://$SERVER_IP:5005/api"
