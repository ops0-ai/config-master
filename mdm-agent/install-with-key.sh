...
# Pulse MDM Agent Installer (LaunchAgent Version)
# Usage: ./install-with-key.sh <enrollment_key> [server_url]
# 
# This installer creates a LaunchAgent that runs with user privileges.
# For full system control (shutdown/restart), use a LaunchDaemon version instead.
...
# Create agent directory in user's Library (avoid conflicts with existing root-owned directory)
AGENT_DIR="$HOME/Library/Application Support/PulseMDM"
mkdir -p "$AGENT_DIR"
...
sed -i '' "s|'http://localhost:5005/api'|'$SERVER_URL'|g" "$AGENT_PATH"
...
if launchctl load "$PLIST_PATH" 2>/dev/null; then
    ...
    if launchctl list | grep -q "com.pulse.mdm.agent"; then
        print_success "✅ Pulse MDM agent is running via LaunchAgent!"
    else
        print_warning "⚠️ LaunchAgent loaded but not running, trying manual start..."
        # Fallback to manual start
        export PULSE_SERVER_URL="$SERVER_URL"
        export PULSE_ENROLLMENT_KEY="$ENROLLMENT_KEY"
        nohup /usr/bin/python3 "$AGENT_PATH" > ~/Library/Logs/pulse-mdm-manual.log 2>&1 &
        sleep 2
        if ps aux | grep -v grep | grep -q "pulse-mdm-agent.py"; then
            print_success "✅ Pulse MDM agent is running manually!"
        else
    ...
    print_warning "⚠️ LaunchAgent failed to load, starting manually..."
    ...
    if ps aux | grep -v grep | grep -q "pulse-mdm-agent.py"; then
        print_success "✅ Pulse MDM agent is running manually!"
        echo ""
        echo "📝 Note: Agent started manually due to LaunchAgent restrictions."
        echo "   The agent will run until you log out or restart."
        echo "   To make it persistent, you may need admin privileges."
    else
        print_error "❌ Failed to start agent manually"
        echo ""
        echo "🔍 Troubleshooting:"
        echo "   • Check logs: tail -f ~/Library/Logs/pulse-mdm-manual.log"
        echo "   • Verify Python: /usr/bin/python3 --version"
        echo "   • Test manually: /usr/bin/python3 '$AGENT_PATH'"
        exit 1
    fi
fi
